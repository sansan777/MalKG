...
class BiLSTM(nn.Module):
	def __init__(self, config):
		...
		if self.use_entity_type:
			...
			self.ner_emb = nn.Embedding(33, config.entity_type_size, padding_idx=0) #First number is the number of NER classifications (I think)
		...
		if self.use_distance:
			self.dis_embed = nn.Embedding(30, config.dis_size, padding_idx=15) #First number is the number of distinct distances
