...
class Config(object):
	def __init__(self, args):
		...
		self.max_length = 16384 #The maximum number of words appearing in a single document
		self.relation_num = 35 #1 more than the number of relations in rel2id... why?
		...
		self.test_epoch = 5 #The number of epochs between validation phases, the model is only saved during the validation phase
		...
		self.batch_size = 5
		self.h_t_limit = 6400 #Greatest number of vertices in any one document, squared

		self.test_batch_size = self.batch_size
		self.test_relation_limit = self.h_t_limit
		self.char_limit = 128 #Maximum number of characters in a single word
		self.dis2idx = np.zeros((16384), dtype='int64') #This determines how many words apart named entities can be. DO NOT CHANGE WITHOUT ADJUSTING THE APPROPRIATE EMBEDDING IN BiLSTM!
		self.dis2idx[1] = 1
		self.dis2idx[2:] = 2
		self.dis2idx[4:] = 3
		self.dis2idx[8:] = 4
		...
		self.dis2idx[4096:] = 13
		self.dis2idx[8192:] = 14
		self.dis_size = 20
	...
	def train(self, model_pattern, model_name):
		...
		for epoch in range(self.max_epoch):
			...
			for data in self.get_train_batch():
				...
				dis_h_2_t = ht_pair_pos+15 #Changed from 10 to 15, as I extended dis2idx from 9 to 14
				dis_t_2_h = -ht_pair_pos+15 #Changed from 10 to 15, as I extended dis2idx from 9 to 14
